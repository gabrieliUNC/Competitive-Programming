...
int main(){
    ...
    while(t--){
        ...
        vector<int> a, b, ans_a, ans_b;
        for(int i = 1; i <= n; ++i) if(a_cnt[i] > 1) a.pb(i);
        for(int i = 1; i <= n; ++i) if(b_cnt[i] > 1) b.pb(i);

        while(k && size(a) && size(b)) {
            ans_a.pb(a.back());
            ans_a.pb(a.back());
            ans_b.pb(b.back());
            ans_b.pb(b.back());
        ...
        for(auto& elt : ans_a) cout << elt << ' ';
        cout << '\n';
        for(auto& elt : ans_b) cout << elt << ' ';
        cout << '\n';
