...
ll solve() {
    int n, m, k; cin >> n >> m >> k;
    vector<ll> a(n), models(m), func(k);
    for(auto& e : a) cin >> e;
    for(auto& e : models) cin >> e;
    for(auto& e : func) cin >> e;
    ...
    ll mid1, mid2, best1 = LLONG_MAX, best2 = LLONG_MAX;
    if((r - l) % 2 == 0) mid1 = (r - l) / 2 + l, mid2 = (r - l) / 2 + l;
    else {
        mid1 = (r - l) / 2 + l;
        mid2 = (r - l) / 2 + l + 1;
    }
    ...
    for(int i = 0; i < m; ++i) {
        ll cur = models[i];
        int pos = distance(begin(func), lower_bound(all(func), mid1 - cur));

        for(int j = max(0, pos - 2); j < min(k, pos + 2); ++j) {
            if(abs(cur + func[j] - mid1) < (abs(best1 - mid1))) best1 = cur + func[j];
        }
        
        pos = distance(begin(func), lower_bound(all(func), mid2 - cur));
        for(int j = max(0, pos - 2); j < min(k, pos + 2); ++j) {
            if(abs(cur + func[j] - mid2) < (abs(best2 - mid2))) best2 = cur + func[j];
        }
    }
    ...
    for(int i = 1; i < size(b); ++i) dif = max(dif, b[i] - b[i - 1]);
    ans = min(ans, dif);
    ...
    for(int i = 1; i < size(c); ++i) dif = max(dif, c[i] - c[i - 1]);
    ans = min(ans, dif);
