...
string ans, top, bottom;
ll dfs(int idx, int i, int j) {
    if(idx == size(ans)) return 1;
    if(mp.count({idx, i, j})) return mp[{idx, i, j}];

    if(j && bottom[i] != ans[idx]) return 0;
    if(!j && top[i] != ans[idx]) return 0;

    ll num = 0;

    if(!j) num += dfs(idx + 1, i, 1);
    num += dfs(idx + 1, i + 1, j);
    return mp[{idx, i, j}] = num;
}
...
int main(){
    ...
    while(t--){
        ...
        for(i, j; i < n, j < n; ++i, ++j) {    
            if((top[i] - '0') > (bottom[j] - '0')) break;
        }

        if(j != n) ans = top.substr(0, i) + bottom.substr(j);
        else ans = top.substr(0, i) + bottom[n - 1];
