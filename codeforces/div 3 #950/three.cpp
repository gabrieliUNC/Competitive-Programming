...
bool solve(){
    long long n, charge, loss, idle; cin >> n >> charge >> loss >> idle;
        vector<int> messages(n);
        for(auto &e : messages) cin >> e;

        long long currTime = 0;

        for(auto m : messages){
            long long time_to_wait = (m - currTime) * loss;
            long long delta = min(time_to_wait, idle);
            currTime = m;
            charge -= delta;
