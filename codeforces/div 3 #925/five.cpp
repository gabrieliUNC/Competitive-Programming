...
int main(){
    ...
    while(t--){
		...
		while(pq.size()) {
			// cout << ans << '\n';
			auto [zeroes, size] = pq.top(); pq.pop();
			// cout << zeroes << ' ' << size << '\n';
			if(turn) {
				pq.push({0, size - zeroes});
				// cout << "Anna: " << size - zeroes << '\n';
			} else {
				if(!pq.size()) { ans = size - zeroes; break; }
				auto [zeroes2, size2] = pq.top(); pq.pop();
				// cout << "Sasha: " << zeroes2 << ' ' << size2 << '\n';
				
				// cout << zeroes2 << ' ' << size2 + size + zeroes << '\n';

				// cout << "==================\n";
				pq.push({zeroes2, size2 + size});
			}
